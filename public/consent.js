...
(function () {
  ...
  function injectStyle() {
    ...
    var css = ''
      ...
      + '.ksm-c-card{background:#fff;color:#1a1a2e;max-width:600px;width:100%;max-height:88vh;overflow:auto;border-radius:20px;padding:1.8rem 1.8rem;box-shadow:0 24px 70px rgba(0,0,0,.45);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;animation:ksmPop .28s cubic-bezier(.2,.8,.3,1.2)}'
      ...
      + '.ksm-sw input:checked + .sl{background:#9b7fe0}'
      + '.ksm-sw input:checked + .sl:before{transform:translateX(15px)}'
      + '.ksm-sw input:disabled + .sl{background:#c9bdec;cursor:not-allowed}'
